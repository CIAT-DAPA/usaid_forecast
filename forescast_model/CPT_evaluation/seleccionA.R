...
transform_raster=function(x){ 
  ...
  mapa_base=raster(nrows=73, ncols=144,xmn=0,xmx=357.5, ymn=-90,ymx=90) # Dimensiones del raster
...
rasterize=function(dates) { 
  ...
  layers_crop=crop(layers,extent(0, 357.5, -25, 25)) ## Realiza el corte para que solo tome el tropico
...
plots=function(x,y){
  ...
  library("rasterVis")
  tiff(paste("C:/Users/AESQUIVEL/Google Drive/new_predictor/optimizaciones/",y,".tiff",sep=""),compression = 'lzw',height = 5,width = 16,units="in", res=150)
  myThemec <- BuRdTheme()
...
precp=read.table('C:/Users/AESQUIVEL/Google Drive/new_predictor/santander.csv',header=T,dec=".",sep=",")
