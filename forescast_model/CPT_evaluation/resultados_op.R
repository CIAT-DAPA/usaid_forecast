...
###
setwd("C:/Users/AESQUIVEL/Desktop/Salidas_corrida_DEF/salidas/salidas_optimo/")



### Transformar un archivo .tsv(CPT) en raster 

# Se crea una función que permita crear un raster a partir de los valores de la tabla depurados
transform_raster=function(x){ 
  # Primero se crea un raster teniendo encuenta la resolución espacial de la tabla .tsv  
  mapa_base=raster(nrows=88, ncols=180,xmn=0,xmx=358, ymn=-88,ymx=88)
...
data_trim=function(Estaciones_C, a){
  stations=Estaciones_C
  stations=stations[-1:-2,]
  year=sort(rep(1981:2013,12))
  month=rep(1:12,length(1981:2013))
  data_station=cbind.data.frame(year,month,stations, row.names=NULL)
  pos=seq(a,dim(data_station)[1],12)
  pos_select=sort(c(pos,pos+1,pos+2))
  data_out=aggregate(data_station[pos_select,-1:-2],by=list(sort(rep(1:(length(pos)),3))),sum)
  data_out_final=na.omit(data_out[,-1])
  years_y=na.omit(year[pos+1])
  data_out_final=data.frame(years_y,data_out_final)
...
ruta="C:/Users/AESQUIVEL/Desktop/Salidas_corrida_DEF/salidas/salidas_optimo/"
### Lectura del shp
colombia=shapefile(paste("C:/Users/AESQUIVEL/Desktop/Salidas_corrida_DEF/salidas/","/colombia/colombia_depts.shp",sep=""))
...
cca_maps<-function(var_ocanoAt, yserie, Estaciones_C, xserie, names_file, ruta,  a, xmin, xmax, ymin, ymax, estaciones_in){
  ...
  ## Gráficos Componentes (revisar)
  
  # realice el grafico de las componentes, (primero se crea un arreglo con las fechas)
  ...
  tiff(paste(ruta,"/cca_maps_o_",names_file,".tif",sep=""), height=300,width=1600,res=100,
...
ymin<-  3
ymax<-  5
length_periodo=c(rep(31,9), rep(30,3))
...
ruta_g="C:/Users/AESQUIVEL/Desktop/Salidas_corrida_DEF/salidas/"
Estaciones_C <- read.delim(paste(ruta_g,"dep/precip_",dep,".txt",sep=""),skip =3, header=T, sep="")
...
for(i in 1:12){
  ...
  SST<-read.table(paste(ruta_g,"ERSST_CPT/",lead[i],".tsv",sep=""),sep="\t",dec=".",skip =3,fill=TRUE,na.strings =-999)
  ## Conversión a raster
  SST=rasterize(SST)
...
GoodnessIndex <- function(ruta_c,dep_f){
  ...
  write.csv(x = GoodnessIndex, file = "GoodnessIndex.csv")
  
  GoodnessIndex$a[GoodnessIndex$a==12]=0
  for(i in 1:length(dep_f)){
    ...
    graph_dep  <- graph_dep  + geom_bar(stat = 'identity', fill="gray60") + ylim(-0.02,0.52)
    ...
    tiff(paste(ruta,"/GoodnessIndex_o_dep_",dep_f[i],".tif",sep=""), height=720,width=1280,res=200,
  ...
  tiff(paste(ruta,"/GoodnessIndex_o_line.tif",sep=""), height=720,width=1280,res=200,
       pointsize=2,compression="lzw")
  print(graph_line)
  dev.off()
}
...
grahs_indicador <- function(ruta_c, tipo){
  ...
  tiff(paste(ruta,"/o_",tipo,"_sim",".tif",sep=""), height=600,width=1280,res=200,
...
summary_ind<-function(ruta_c, tipo){
  ...
  #sitios<-c("Lorica","Yopal","Ibagué","La Unión","Espinal","Cereté")
  ...
  tiff(paste(ruta,"/ao_",tipo,".tif",sep=""), height=800,width=1800,res=150,
       compression="lzw")
...
grahs_indicador_r <- function(ruta_r, tipo){
  ...
  # Guarde el gráfico en la ruta de salidas (ruta)
  tiff(paste(ruta,"/retro_or_",tipo,"_sim",".tif",sep=""), height=600,width=1280,res=200,
       pointsize=2,compression="lzw") # height=720,
  print(box)
  dev.off()
  return(datos2)} # la función devuelve la trama de datos en caso de ser necesaria
...
ruta_r= "C:/Users/AESQUIVEL/Desktop/Salidas_corrida_DEF/salidas/salidas_optimo/retroactive/"
...
summary_ind_r<-function(ruta_r, tipo){
  ...
  tiff(paste(ruta,"/retroa_o_",tipo,".tif",sep=""), height=800,width=1800,res=150,
       compression="lzw")
  # layout(matrix(c(1,1,1,1,1,1,1,1,2), 1,9, byrow = TRUE))
  plot(ROC_B~f1,data=res.shape2,type="n",xlab=" ",
       ylab=tit,xaxt="n",bty="n", ylim=lim)#
...
for(i in  1:length(tipo)){
  summary_ind_r(ruta_r = ruta_r, tipo = tipo[i])  
}
...
lead="DEF"
  ...
  cat_prob<-categorias_dep(Estaciones_C,ruta_r,lead,dep,a)
  if(a ==12){
  ...
  for(i in 1:6){
    Estaciones_C <- read.delim(paste(ruta_g,"dep/precip_",dep[i],".txt",sep=""),skip =3, header=T, sep="")
  ...
  dep_aciertosD <- function(ruta_r, a, Estaciones_C, lead){
    ...
    for(i in 1:(length(data)-1)){
      ...
      porcentaje[i] <- sum(estacion_data$cumple)
      RMSE[i] <- sqrt(sum((estacion_data$predictions-estacion_data$obs)^2)/dim(estacion_data)[1])
    }
    
    aciertos<-list(por_deter=porcentaje, RMSE=RMSE)
    #, RMSE=RMSE)
    return(aciertos)}
