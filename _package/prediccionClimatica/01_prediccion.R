...
download.cpt=function(dir_save,month,year){ 
  ...
  for(i in 1:6){
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    #  > prueba que el archivo gzip no esté dañado, sino intenta descargarlo de nuevo
    # ----------------------------------------------------------------------------------------------------------------------
    gunzipTestVal = 1
...
path_prob <- paste0(dir_probForecast,"/",format(Sys.Date(),"%Y%m%d"),"_prob.csv")

write.csv(prob,paste0(dir_probForecast,"/",format(Sys.Date(),"%Y%m%d"),"_prob.csv"),row.names = F)
